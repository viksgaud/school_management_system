        ...
        <div class="row wrapper min-vh-100 flex-column flex-sm-row">
            ...
            <main class="col bg-faded py-3">
                <div class="container">
                    ...
                    <h2 class="text-center" style="margin-top: 10px;">Assign Subject to class</a></h2>
                    <br>
                    <br>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form id="myForm" method="post" action="" class="form-horizontal">
                                        ...
                                        <input type="hidden" name="class_name" value="0">
                                        <input type="hidden" name="sub_name" value="0">
                                        <div class="form-group">

                                            <label for="">Class</label>
                                            <select class="form-control" id="find_class" name="className" required>
                                                <option value="" disabled selected name="option">Select Class</option>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="find_sub" name="subName" required>
                                                <option value="" disabled selected name="option">Select Subject</option>
                    ...
                    <div class="text-center">
                        <button type="button" id="add_button" class="btn btn-info btn-lg add_button"
                            data-toggle="modal">Assign
                            Subject to Class</button>
                    </div>
                </div>
            </main>
            ...
            $(document).ready(function() {
                ...
                $('#add_button').click(function() {
                    ...
                    $('#myModal').modal('show');
                    $('#myModal').find('.modal-title').text("Assign");
                    $('#myForm').attr('action',
                        '<?php echo base_url(); ?>admin/Ast_cl/insertData');
                ...
                $('#btnSave').click(function() {
                    ...
                    // $classNames = $('#find_class').attr('data');

                    // $('input[name=class_name]').val($classNames);

                    // alert($('#find_class').text());
                    var url = $('#myForm').attr('action');

                    var data = $('#myForm').serialize();
                    // var name = $('input[name=name]');
                    var className = $('input[name=className]');
                    var sub = $('input[name=subName]');
                ...
                $('#showData').on('click', '.edit', function() {
                    ...
                    $('#myModal').modal('show');
                    $('#myModal').find('.modal-title').text("Edit Assign");
                    $('#myForm').attr('action',
                        '<?php echo base_url(); ?>admin/Ast_cl/updateData');

                    $.ajax({
                        url: '<?php echo base_url(); ?>admin/Ast_cl/editData',
                        type: 'ajax',
                        method: 'get',
                        data: {
                            id: id
                        ...
                        success: function(data) {
                            ...
                            $('input[name=className]').val(data.class);
                            $('input[name=subName]').val(data.subject);
                ...
                $('#showData').on('click', '.delete', function() {
                    ...
                    $('#btnDelete').unbind().click(function() {
                        $.ajax({
                            url: '<?php echo base_url(); ?>admin/Ast_cl/deleteData',
                            type: 'ajax',
                            method: 'get',
                            data: {
                                id: id
                ...
                function showFormData() {
                    ...
                    $.ajax({
                        url: '<?php echo base_url(); ?>admin/Ast_cl/showClass',
                        type: 'ajax',
                        asynk: false,
                        dataType: 'json',
                        success: function(data) {
                            ...
                            for (i = 0; i < data.length; i++) {
                                ...
                                html +=
                                    '<option value="' + data[i].id + '" data="' + data[i]
                    ...
                    $.ajax({
                        url: '<?php echo base_url(); ?>admin/Ast_cl/showSubject',
                        type: 'ajax',
                        asynk: false,
                        dataType: 'json',
                        success: function(data) {
                            ...
                            for (i = 0; i < data.length; i++) {
                                html +=
                                    '<option value="' + data[i].id + '" data-foo="' + data[i]
                                    .subjectName + '">' + data[i].subjectName +
                                    '</option>';
                                // $('input[name=sub_name]').val(data[i].subjectName);
                ...
                function showAllData() {
                    ...
                    $.ajax({
                        url: '<?php echo base_url(); ?>admin/Ast_cl/showData',
                        type: "ajax",
                        asynk: false,
                        dataType: 'json',
                        success: function(response) {
                            ...
                            for (i = 0; i < response.length; i++) {
                                $('#showData').html(
                                    ...
                                    response[i].id + '" class_name="' + response[i].class +
                                    '">Edit</td>' +
                                    '<td>' +
